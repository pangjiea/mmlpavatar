...
try:
    # Prefer vendored SSS optimizer if available
    from 3D-student-splatting-and-scooping.utils.sghmc import AdamSGHMC  # type: ignore
except Exception:
    # Fallback: local copy of AdamSGHMC (minimal import to avoid path issues)
    import torch
    import numpy as np
    from torch.optim import Optimizer
    import math

    class AdamSGHMC(Optimizer):  # type: ignore
        name = "SGHMC"
        def __init__(self, params, lr: float=1e-2, betas=(0.9, 0.999), eps=1e-8, weight_decay=0, mdecay: float=0.01, wd: float=0.00002, scale_grad: float=1., mdecay_burnin: float=0.01, burnin_iterations: int=7000) -> None:
            if lr < 0.0:
                raise ValueError(f"Invalid learning rate: {lr}")
            defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, mdecay=mdecay, wd=wd, scale_grad=scale_grad, mdecay_burnin=mdecay_burnin, burnin_iterations=burnin_iterations)
